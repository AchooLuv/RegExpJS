  ...
  <div class="erc-con">
    ...
    <p>在掌握了正则表达式应用的基础知识和了解了回溯的细节后，现在来全面学习正则表达式应用到目标字符串的过程：
      <ul>
        ...
        <li>传动开始：<span class="ins">正则引擎将正则引擎“定位”到目标字符串的其实位置。</span></li>
        <li>元素检测：<span class="ins">正则引擎开始测试正则表达式和文本，依次测试表达式的各个元素。</span></li>
        <li>寻找匹配结果：<span class="ins">若匹配到一个结果，传统型NFA会“锁定”当前状态，报告匹配成功。</span></li>
        <li>传动装置的驱动过程：<span class="ins">若未找到匹配，传动装置就会驱动引擎，从文本的下一个字符开始新一轮的尝试（回到第三步）。</span></li>
        <li>匹配彻底失败：<span class="ins">如果从目标字符串的每个字符开始尝试都失败了，就会报告匹配彻底失败。</span></li>
      </ul>
    </p>
